...
function HomeIcon({ className = 'h-6 w-6', filled }) {
  // النسبُ مقيسةٌ من لقطةِ شاشةٍ لزرِّ إنستغرام لا مقدَّرةٌ بالعين: عرضُ البيتِ
  // إلى ارتفاعِه ١٫٣ إلى واحد — أعرضُ منه ارتفاعاً — والسقفُ والجدرانُ نصفانِ
  // متساويان تماماً. كلُّ محاولاتي قبلَها كانت شبهَ مربّعةٍ فبدت بيتاً آخر.
  //
  ...
  const body = 'M10.96 5.5A1.7 1.7 0 0 1 13.04 5.5L20.99 11.61A1.3 1.3 0 0 1 21.5 12.64L21.5 17A2.3 2.3 0 0 1 19.2 19.3L4.8 19.3A2.3 2.3 0 0 1 2.5 17L2.5 12.64A1.3 1.3 0 0 1 3.01 11.61Z';
  ...
  const doorFill = 'M9.8 19.3L9.8 16.2A2.2 2.2 0 0 1 14.2 16.2L14.2 19.3Z';
  const doorLine = 'M9.8 19.3V16.2A2.2 2.2 0 0 1 14.2 16.2V19.3';
  if (filled) {
    return (
...
function ReelsIcon({ className = 'h-6 w-6', filled }) {
  ...
  const play = 'M10.2 8.5 16.3 12 10.2 15.5Z';
  if (filled) {
    return (
      <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden="true">
        ...
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d={`M9.1 2.9h5.8c3.4 0 6.2 2.8 6.2 6.2v5.8c0 3.4-2.8 6.2-6.2 6.2H9.1c-3.4 0-6.2-2.8-6.2-6.2V9.1c0-3.4 2.8-6.2 6.2-6.2Z ${play}`}
        />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <rect x="2.9" y="2.9" width="18.2" height="18.2" rx="6.2" />
      <path d={play} />
    </svg>
  );
}
