...
export default function CartDrawer() {
  ...
  // نجمّع العناصر حسب رقم واتساب المتجر لإرسال طلب لكل متجر
  const groups = {};
  items.forEach((i) => {
  ...
  const checkout = () => {
    Object.entries(groups).forEach(([wa, groupItems]) => {
      const url = buildWhatsappOrder(wa === 'none' ? '' : wa, groupItems, i18n.language);
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {items.map((i) => (
                <div key={i.id} className="glass flex gap-3 p-3">
                  <img
                    src={i.imageUrl || 'https://placehold.co/64x64/121214/d4af37?text=%F0%9F%91%97'}
                    alt={i.name}
            ...
            <div className="border-t border-gold-400/15 p-4">
              ...
              <button onClick={checkout} className="btn-whatsapp w-full">💬 {t('cart.orderWhatsapp')}</button>
              <button onClick={clear} className="mt-2 w-full text-center text-xs text-stone-400 hover:text-red-300">
                {t('cart.clear')}
              </button>
            </div>
          </>
