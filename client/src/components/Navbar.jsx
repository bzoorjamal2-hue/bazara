...
export default function Navbar() {
  ...
  const handleLogout = async () => {
    setMenuOpen(false);
    // الترتيب هنا مقصود، وكان معكوساً فيُحدث «لمحة» صفحةِ الدخول:
    //
    // logout() يمسح المستخدم فوراً ثمّ ينتظر الخادم حتى ٢٫٥ ثانية. وبانتظارِ
    // await كنّا ما نزال على /dashboard المحميّ، فيرانا ProtectedRoute بلا
    // مستخدمٍ ويرمينا إلى /login — ثمّ يصل navigate('/') فننتقل للواجهة.
    // فتظهر الشاشتان تِباعاً.
    //
    // الآن: نبدأ الخروج (يمسح الحالة تزامنياً) وننتقل بنفس النبضة، فتُجمَّع
    // التحديثتان برسمةٍ واحدة: الموقعُ الجذرُ والمستخدمُ فارغ معاً — فلا
    // يمرّ الرسمُ بحالة «داخل صفحةٍ محميّة بلا مستخدم» أصلاً. ثمّ ننتظر مسح
    // الكوكي، فهو ما يوثّق الطلب التالي وبلا مسحه يعود المستخدم داخلاً.
    const done = logout();
    // الجذر لا /shop: صار صفحة المنصّة بالمتصفّح والتطبيق معاً.
  ...
  const menuGroups = !user ? [] : isAdmin
    ...
    : [
        ...
        { id: 'grow', title: t('dashboard.menu.grow'), items: [
          ...
          { key: 'adStudio', label: t('adStudio.nav'), Icon: PaletteIcon },
        ] },
        { id: 'account', title: t('dashboard.menu.account'), items: [
  ...
  return (
    ...
    >
      <nav
        className={`app-navbar bz-page relative flex justify-center py-2.5 ${noAnim ? 'bz-noanim transition-none' : 'transition-shadow duration-300'} ${scrolled ? 'shadow-md' : ''}`}
        style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 0.625rem)' }}
      >
        <div className="w-full">
        <div className={`relative flex w-full items-center justify-between transition-[height] duration-300 ease-out ${scrolled ? 'h-11' : 'h-12'}`}>
          ...
          >
            <span className={`font-display font-extrabold tracking-wide text-wine transition-[font-size] duration-300 ease-out ${scrolled ? 'text-[20px]' : 'text-2xl sm:text-[28px]'}`}>Bazara</span>
            <span className={`flex items-center gap-1.5 overflow-hidden text-[10px] font-bold tracking-[0.35em] text-wine/45 ${noAnim ? '' : 'transition-all duration-300 ease-out'} ${scrolled ? 'mt-0 max-h-0 opacity-0' : 'mt-1 max-h-4 opacity-100'}`}>
