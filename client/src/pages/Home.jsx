  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (touch.current.horiz) {
        e.preventDefault();
        // «التالي» يميناً بالعربيّةِ ويساراً بالإنجليزيّة. والمقاومةُ عندَ الطرفَين:
    ...
    const onEnd = (e) => {
      ...
      if (touch.current.horiz && Math.abs(dx) > threshold) {
        // يتبعُ الإصبعَ باتّجاهِ اللغة: يميناً = التالي بالعربيّة، ويساراً = التالي
    ...
    <section className="relative">
      ...
      >
        {/* اتّجاهُ الحركةِ يتبعُ اللغةَ كترتيبِ صفحاتِ الكتابِ العربيّ:
            ‏العربيّةُ — ‎row-reverse: الأولى يميناً والتاليةُ إلى يسارِها، فيمشي
            الشريطُ يميناً وتدخلُ التاليةُ من اليسار — كتقليبِ صفحةٍ عربيّة.
            ‏الإنجليزيّةُ — ‎row: الأولى يساراً والتاليةُ إلى يمينِها، والعكسُ صحيح.
