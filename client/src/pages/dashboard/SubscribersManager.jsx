...
function SubRow({ s, onDeleted, onUpdated, onOpen }) {
  ...
  const statusBadge = s.isAdmin ? (
    <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold shadow-sm" style={{ background: '#999795', color: '#1E1D1C' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {chips.map((c) => (
              <button
                ...
                style={filter === c.key ? { background: '#999795', color: '#1E1D1C' } : undefined}
              >
