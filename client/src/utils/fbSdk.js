...
export function startFbLogin({ appId, configId, graphVersion = 'v21.0' }) {
  ...
  const p = new URLSearchParams({
    ...
    response_type: 'code',
    state,
    // فيسبوك يتذكّر آخر من دخل في هذا المتصفّح، فيعرض «تريد المتابعة كـفلان؟» بلا
    // بابٍ لاختيار حسابٍ آخر. وعلى منصّةٍ متعدّدةِ المتاجر هذا عطلٌ لا راحة: التاجرةُ
    // الثانيةُ تجدُ نفسَها تربطُ حسابَ الأولى، ومن غيّر حسابَه لا يستطيع تصحيحَه.
    // reauthenticate يطلبُ الدخولَ من جديدٍ في كلِّ مرّة — فيربطُ كلٌّ حسابَه هو.
