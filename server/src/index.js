...
//
// كان الحدُّ ٣٠٠ لكلِّ عنوانٍ في ربعِ ساعة، وهو يكفي زائراً يتصفّح. لكنّ لوحةَ
// التاجرةِ تسألُ الخادمَ بانتظام: صندوقُ رسائلِ إنستغرام يتحدّثُ كلَّ ثوانٍ لتصلَ
// الرسالةُ وهي تنظر. فكانت اللوحةُ تبلغُ الحدَّ بعد دقائقَ من فتحِ محادثة، ثمّ يردُّ
// الخادمُ ٤٢٩ على **كلِّ** طلبٍ حتّى تنتهي النافذة — فيبدو التطبيقُ معطّلاً بلا سبب.
//
// فصار الحدُّ حدَّين: زائرٌ بلا جلسةٍ كما كان، وصاحبةُ جلسةٍ أوسعُ بما يسمحُ
// بالتحديثِ المنتظمِ ولا يسمحُ بإساءةٍ (نحوَ طلبٍ ونصفٍ في الثانية).
const hasSession = (req) => Boolean(req.cookies?.token || req.headers.authorization);
app.use(
  rateLimit({
    windowMs: 15 * 60 * 1000,
    max: (req) => (hasSession(req) ? 1500 : 300),
...
END $$;`,
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    ...
    // هامشُ المفاصلة: كم شيقلاً تنزلُ البائعةُ عن سعرِ القطعةِ حين تُفاصَل. عشرةٌ
    // للمنصّةِ كلِّها، ويُحسَبُ وقتَ الردِّ فتأخذُه القطعُ الجديدةُ بلا أن يُكتَبَ فيها.
    "ALTER TABLE stores ADD COLUMN IF NOT EXISTS bot_haggle_margin NUMERIC(10,2) NOT NULL DEFAULT 10;",
    // من أينَ جاءت هذه المحادثة: دايركت إنستغرام أم ماسنجر الصفحة. كلاهما يصلُ
    // بنفسِ الـwebhook وبنفسِ توكنِ الصفحة، لكنّ التاجرةَ تختارُ لكلٍّ منهما على
    // حدة، والردُّ يجبُ أن يعرفَ أيَّهما ليحترمَ اختيارَها.
    "ALTER TABLE ig_conversations ADD COLUMN IF NOT EXISTS channel VARCHAR(16) NOT NULL DEFAULT 'instagram';",
    // متى أرسلَتِ البائعةُ آخرَ رسالةٍ بهذه المحادثة. حارسُ «التاجرةُ على الشاشة»
    // يقيسُ عليه، فلا يحسبُ صدى رسالةِ البائعةِ نفسِها ردّاً يدويّاً من التاجرة.
    "ALTER TABLE ig_conversations ADD COLUMN IF NOT EXISTS bot_sent_at TIMESTAMPTZ;",
    // الإعلانُ الذي جاءت منه الزبونة. ميتا تُرسلُه مع أوّلِ رسالةٍ من إعلانِ
    // «راسِلنا» داخلَ حقلِ messages نفسِه (بلا اشتراكٍ جديد): عنوانُ الإعلانِ
    // وصورتُه ومعرّفُه. وكنّا نرميه، فتسألُ زبونةٌ ضغطت إعلانَ فستانٍ «كم السعر؟»
    // فتردُّ البائعةُ «أي قطعة؟» — وكأنّ المحلَّ لا يعرفُ ما أعلنَ عنه.
    "ALTER TABLE ig_conversations ADD COLUMN IF NOT EXISTS ad_ref JSONB;",
    // ═══ مصنع الإعلانات ═══
