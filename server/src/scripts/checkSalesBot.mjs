...
// ═ سطرُ التوفّر: المتوفّرُ أوّلاً ═
//
// كان يسردُ كلَّ لونٍ بكلِّ نمرةٍ على نسقٍ واحد، فدُفِنَ المتوفّرُ الوحيدُ تحتَ
// ثمانيةَ عشرَ «نفدت». فقالت البائعةُ لزبونةٍ أعطتها اسمَها ورقمَها وعنوانَها:
// «نفدت بمقاس ٣٨» — و٣٨ موجودة. طلبٌ مكتملٌ رُدَّ على بابِه.
{
  const rama = { color_stock: { بني: { 36: 0, 38: 0 }, كحلي: { 36: 1, 38: 1, 40: 0 } } };
  const sl = stockLine(rama);
  t('المتوفّرُ يُذكَرُ أوّلاً وصراحةً', sl.startsWith('المتوفّر: كحلي(36، 38)'), sl);
  t('ولونٌ نفدَ كلُّه يُذكَرُ باسمِه بلا تعداد', /ونفد:.*بني/.test(sl) && !/بني\(/.test(sl), sl);
  t('ونمرةٌ نفدت من لونٍ متوفّرٍ تبقى مذكورة', sl.includes('كحلي(40)'), sl);
  t('والمتوفّرُ يسبقُ النافدَ دائماً', sl.indexOf('كحلي(36') < sl.indexOf('ونفد'), sl);
  t('ونفادُ الكلِّ يُقالُ بكلمةٍ واحدة',
    stockLine({ color_stock: { بني: { 36: 0 }, اسود: { 38: 0 } } }) === 'نفد كلُّه');
  const bySize = stockLine({ size_stock: { 36: 0, 38: 3, 40: 0 } });
  t('وبلا ألوانٍ كذلك', bySize === 'المتوفّر: 38:3 — ونفد: 36، 40', bySize);
  t('وقطعةٌ بعدّادٍ واحدٍ لا تتغيّر', stockLine({ stock: 5 }) === 'متوفّر (5)');
  // والقطعُ الحقيقيّةُ بالمتجرِ لا تكذبُ عن نفسِها
  for (const p of rows) {
    // القطعُ ذاتُ العدّادِ الواحدِ لا تتبّعُ نمرَها أصلاً («متوفّر (20)») — نمرُها
    // تأتي من عمودِ المقاساتِ لا من مخزونٍ لكلِّ نمرة، فلا شيءَ هنا يُفحَص.
    const tracks = Object.keys(p.color_stock || {}).length || Object.keys(p.size_stock || {}).length;
    if (!tracks) continue;
    const line = stockLine(p);
    const avail = sizesOf(p);
    if (!avail.length) continue;
    t('«' + String(p.name).slice(0, 18) + '»: كلُّ نمرةٍ متوفّرةٍ مذكورةٌ بقسمِ المتوفّر',
      avail.every((sz) => line.split('— ونفد:')[0].includes(String(sz))), line.slice(0, 80));
  }
}
...
// «راعيني» قالها الزبونُ مرّتينِ فلم تتحرّكْ درجةُ المفاصلةِ أصلاً — لم تكن
// بالقاموس. واللهجةُ الفلسطينيّةُ أوسعُ من أيِّ قائمة، فالنموذجُ يسدُّ ما بعدَها.
for (const w of ['راعيني', 'راعيني شوي', 'ما بتراعيني؟', 'زبطني بالسعر', 'ظبطني شوي',
  'شيل اشي من السعر', 'كسرلي السعر', 'رخصلي', 'معقولة هالسعر', 'كتير علي']) {
  t('«' + w + '» مفاصلة', haggleIntent(w));
}
for (const w of ['حلوة كتير', 'بدي اطلب', 'شو الالوان', 'بدي اشوف الاخضر', 'وين محلكم']) {
  t('«' + w + '» ليست مفاصلة', !haggleIntent(w));
}
...
H('١٤) اللهجةُ فلسطينيّةٌ لا خليطَ لهجات');
{
  ...
  t('و«خدي اسمك» كذلك', fixed('بس خدي اسمك الكامل ورقم التلفون', 'احكيلي اسمك', 'خدي اسمك'));
  t('و«خد رقمها» كذلك', fixed('خد رقمها والعنوان', 'احكيلي رقمها', 'خد رقمها'));
  t('«لسه» مصريّةٌ ⇒ «لسّا»', fixed('لسه معنا مقاسين', 'لسّا', 'لسه'));
  // والباءُ لا تُبطِلُ «بدّي» ولا «بدّه» — أفلسطينيُّ كلمتين بالدنيا
  t('«بدي» و«بده» لا تُمَسّان', palestinize('بدي إشي وبده يروح') === 'بدي إشي وبده يروح');
  t('و«خدي» وحدَها تبقى («خدي راحتك»)', palestinize('خدي راحتك') === 'خدي راحتك');
