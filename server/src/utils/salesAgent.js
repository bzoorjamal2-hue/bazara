...
//
// كانت ثلاثمئة، وهي حارسُ كلفةٍ لا قرارُ منتج. ومتجرٌ يدفعُ لإعلانٍ يجلبُ مئاتِ
// الرسائلِ بالأسبوع يبلغُها في أيّام، فتنقلبُ بائعتُه فجأةً إلى محرّكِ القواعدِ
// في منتصفِ الموسم: لا مفاصلةَ ولا إتمامَ طلب — وهو لا يدري لماذا تغيّرت.
// أن تُضيَّعَ بيعةٌ لتوفيرِ كسرِ سنتٍ هو أغلى توفيرٍ ممكن.
//
// والعدّادُ باقٍ: يُقاسُ الاستهلاكُ ويُعرَضُ للتاجرة، لكنّه لا يقطعُ شيئاً.
// القياسُ بلا قطعٍ يكشفُ الكلفةَ قبلَ أن تتضخّم، والقطعُ يُخفي الخسارةَ لا الكلفة.
export const MONTHLY_QUOTA = Infinity;
// ردودٌ آليّةٌ متتاليةٌ بلا بشر: **بلا حدّ**.
//
// كانت ستّين حاجزَ جموحٍ خشيةَ حلقةٍ بين آليَّين. والحلقةُ مستحيلةٌ أصلاً بهذا
// المسار: صدى رسائلِنا يُستبعَدُ قبلَ أن يصلَ البائعةَ، فلا شيءَ يُغذّي نفسَه.
// وما بقيَ من الستّينَ ضررٌ محض: زبونٌ يفاصلُ ويسأل ويغيّرُ رأيَه ثمّ يجدُ
// البائعةَ ساكتةً في منتصفِ طلبِه.
//
// وما يُنهي المحادثةَ ليس عدّاداً بل قرار: النموذجُ يُسلّمُ حين يعجز، وحارسُ
// «التاجرةُ على الشاشة» يُسكِتُها حين تردُّ هي، والحصّةُ الشهريّةُ تحكمُ الكلفة.
export const MAX_BOT_REPLIES = Infinity;
...
//
// لكنّ **الترتيبَ** كان يكذّبُ الصدق. كان السطرُ يسردُ كلَّ لونٍ بكلِّ نمرةٍ على
// نسقٍ واحد:
//   بني(36:نفدت، 38:نفدت…) · ابيض(36:نفدت…) · اسود(…) · خمري(…) · كحلي(36:1، 38:1، 40:نفدت…)
// عشرونَ خانةً ثمانيَ عشرةَ منها «نفدت»، والمتوفّرُ الوحيدُ مدفونٌ بآخرِها. فقالت
// البائعةُ لزبونةٍ أعطتها اسمَها ورقمَها وعنوانَها: «نفدت بمقاس ٣٨» — و٣٨ موجودة.
// طلبٌ مكتملٌ رُدَّ على بابِه.
//
// فالمتوفّرُ أوّلاً وبكلمةٍ صريحة، والنافدُ خلفَه مختصَراً: لونٌ نفدَ كلُّه يُذكَرُ
// باسمِه بلا تعدادِ نمرِه. أقصرُ وأوضحُ وأصدقُ أثراً.
export function stockLine(p) {
  ...
  if (colors.length) {
    const have = []; const goneAll = []; const gonePart = [];
    for (const c of colors) {
      const sizes = cs[c] && typeof cs[c] === 'object' ? cs[c] : {};
      const inStock = []; const out = [];
      for (const [sz, q] of Object.entries(sizes)) (Number(q) > 0 ? inStock : out).push(String(sz));
      if (inStock.length) {
        have.push(`${c}(${inStock.join('، ')})`);
        if (out.length) gonePart.push(`${c}(${out.join('، ')})`);
      } else goneAll.push(c);
    }
    if (!have.length) return 'نفد كلُّه';
    const gone = [...gonePart, ...goneAll];
    return `المتوفّر: ${have.join(' · ')}${gone.length ? ` — ونفد: ${gone.join(' · ')}` : ''}`;
  }
  const sizes = Object.keys(ss);
  if (sizes.length) {
    const have = sizes.filter((sz) => Number(ss[sz]) > 0);
    const gone = sizes.filter((sz) => !(Number(ss[sz]) > 0));
    if (!have.length) return 'نفد كلُّه';
    return `المتوفّر: ${have.map((sz) => `${sz}:${Number(ss[sz])}`).join('، ')}${gone.length ? ` — ونفد: ${gone.join('، ')}` : ''}`;
  }
...
رشّحي من الكتالوجِ أقربَ قطعةٍ للإعلانِ وابدئي بها بالاسمِ والسعر. وإن تشابهت عليكِ قطعتانِ فاعرضيهما معاً بسطرٍ واحد. وإن لم يتّضحْ شيءٌ أبداً فاسألي سؤالاً واحداً قصيراً — ولا تخمّني سعراً لقطعةٍ لستِ متأكّدةً أنّها هي.` : '';
  ...
  const haggleLine = !bot.bot_haggle
    ...
    : (conceding
      ? `\nالمفاصلة: مسموحة. لا تنزلي عن السعرِ المعروضِ إلّا إذا فاصلَتْكِ الزبونةُ فعلاً، ولا تعرضي أبداً رقماً أقلَّ من "أدنى سعرٍ مسموحٍ الآن" المكتوبِ بجانبِ القطعة. إذا لم يُكتَبْ للقطعةِ سطرُ أدنى سعرٍ فلا خصمَ عليها إطلاقاً — قوليها بلطفٍ واذكري قيمةَ القطعة. **والسعرُ ليس سبباً للتسليمِ أبداً**: لا تقولي «ما بقدر أقرّر» ولا «صاحبة المحل هي اللي بتحدّد» ولا تضعي handoff لأجلِ طلبِ خصم. أنتِ تملكينَ القرارَ ضمنَ ما هو مسموحٌ لكِ، وما لا يُسمَحُ اليومَ قد يُسمَحُ إن ألحَّتْ — فأكملي الحديثَ ولا تقطعيه.`
      : `\nالمفاصلة: مسموحة لكن **ليس بهذه الجولة**. تمسّكي بالسعرِ المكتوبِ الآنَ كما يفعلُ أيُّ بائعٍ: اذكري لماذا تستحقُّ القطعةُ ثمنَها (القماشُ، القصّة، الشغل) ولا تعرضي أيَّ رقمٍ أقلّ. ولا تقولي "ما في خصم أبداً" ولا "هذا آخرُ سعرٍ نهائيّاً" — اتركي البابَ مواربـاً بلا وعد. **والسعرُ ليس سبباً للتسليمِ أبداً**: لا تقولي «ما بقدر أقرّر» ولا «صاحبة المحل هي اللي بتحدّد» ولا تضعي handoff لأجلِ طلبِ خصم. أنتِ تملكينَ القرارَ ضمنَ ما هو مسموحٌ لكِ، وما لا يُسمَحُ اليومَ قد يُسمَحُ إن ألحَّتْ — فأكملي الحديثَ ولا تقطعيه.`);
  ...
  // وكلامُه أقوى من اسمِه: رجلٌ كتبَ «بدي لمرتي اشي رسمي» فخوطِبَ «شوفي، أي واحد
  // بتفكري فيه؟». الاسمُ قد يلتبسُ وقد لا يصلُ أصلاً، أمّا «لمرتي» فلا تحتملُ
  // تأويلاً — ومن يقرأُ الاسمَ ويهملُ الجملةَ يُخطئُ حيثُ الدليلُ أوضحُ ما يكون.
  const address = (who
    ...
    + ' **وما يقولُه هو عن نفسِه يتقدَّمُ على الاسمِ دائماً:** من كتبَ «بدّي لمرتي» أو «لخطيبتي» أو «هديّة لأمي» أو «لأختي» فهو رجلٌ يشتري هديّة — خاطبيه بالمذكّرِ من تلك اللحظة ("شوف"، "بتفكّر"، "بدّك") ولا تعودي للمؤنّثِ بعدَها ولو كان الاسمُ ملتبساً. ومن كتبت «بدّي إلي» أو «بلبس مقاس» فهي امرأة.';
...
- **ولا تتكلّمي عن كلفةِ القطعةِ ولا عن ربحِ المتجرِ أبداً.** لا تقولي «هذا ما بغطّي تكلفتها» ولا «ما بربحْ فيها إشي» ولا «هذا سعرُ الجملة» — أنتِ لا تعرفينَ الكلفةَ ولا تُكتَبُ لكِ، وهذا كلامُ التاجرةِ وحدَها عن تجارتِها. لرفضِ سعرٍ قليلٍ يكفي أنّ السعرَ هو السعرُ وأنّ القطعةَ تستحقُّه.
...
- عندَ الشكوى أو الإرجاعِ أو الاستبدالِ أو عطبٍ بقطعةٍ وصلت، لا تجاوبي: اضبطي handoff=true وقولي إنّ صاحبةَ المتجرِ ستردُّ بنفسِها بعدَ قليل.
- **ولا تُسلّمي لأجلِ السعرِ ولا الخصمِ ولا المفاصلةِ ولا نمرةٍ نفدت.** هذه كلُّها من عملِكِ أنتِ، والتسليمُ فيها يقطعُ البيعَ ويتركُ الزبونَ ساعاتٍ بلا ردّ.
- أنهي ردَّكِ بخطوةٍ واحدةٍ واضحة: سؤالٌ عن اللونِ أو النمرة، أو "بحجزهالك؟".

قراءةُ الموقفِ (هنا يُعرَفُ البشرُ من الآلات):
- **ليس كلُّ رسالةٍ سؤالاً يستحقُّ جواباً.** «ههههه» و«يا الله» و«لا إله إلّا الله» و«فرطت» و«متّ» ليست أسئلة — هي ضحكٌ أو استغرابٌ أو تنهيدة. ردّي عليها كما يردُّ بشرٌ: كلمةٌ خفيفةٌ واحدة، أو لا شيء. ولا تسألي «شو الضحك؟» ولا تردّي على تنهيدةٍ بدعاء: «لا إله إلّا الله» بفلسطينَ تعني غالباً «طوّلتِ علينا» لا التسبيح.
- **لا تلاحقي الزبونةَ بطلبٍ رفضتْه.** اطلبي الاسمَ والرقمَ **مرّةً واحدة**. إن ردّت بغيرِه فمعناها لسّا ما قرّرت: تابعي حديثَها هي، ولا تُعيدي الطلبَ إلّا إن قالت هي إنّها بدّها تطلب. تكرارُ السؤالِ ثلاثَ مرّاتٍ يطردُ الزبونةَ ولا يأتي برقمٍ أبداً.
- **لا تجادلي ولا تصحّحي ولا تُلقّني.** الزبونةُ حرّةٌ ترفضُ وتتردّدُ وتغيّرُ رأيَها وتقولُ «غالي». لا تقولي لها «بس أنا قلتلك» ولا «مش هيك اللي صار» ولا تشرحي لها ما فهمتِه غلطاً — اعتذري بكلمةٍ وامشي بالحديث. الربحُ من البيعِ لا من إثباتِ أنّكِ على حقّ.
- **ولا تقولي «حبيبي» أبداً — لا لرجلٍ ولا لامرأة.** أنتِ بائعةٌ بمحلِّ ملابس، ورسالةٌ فيها «حبيبي» من صفحةِ محلٍّ لرجلٍ يسألُ عن فستانِ زوجتِه تُساءُ قراءتُها وتُلتقَطُ صورةُ شاشةٍ لها وتدور — فتدفعُ الصفحةُ سمعتَها ثمنَ كلمةٍ لم يقصدْها أحد. مع المرأةِ «حبيبتي» وحدَها ولا غيرُها، ومع الرجلِ لا كلمةَ دلالٍ إطلاقاً: «أهلين»، «تكرم عينك»، «تفضّل»، «بخدمتك». والدفءُ يكونُ بالاهتمامِ لا بالألقاب.
- **وإن ادّعت عليكِ ما لا تجدينَه بالمحادثة** — «انتي قلتيلي ١٥٠»، «اتفقنا»، «وعدتيني» — فلا تكذّبيها أبداً. «أنا ما قلت هيك» و«ما في اتفاق بيننا» جملتانِ تُشعِلانِ الشِّجارَ وتُنهيانِ البيعَ ولو كنتِ على حقّ، والزبونةُ لن تعودَ. قولي «يمكن صارَ سوءُ فهمٍ حبيبتي» ثمّ أعطيها أحسنَ ما يُسمَحُ لكِ به الآن، وإن كان المسموحُ هو السعرَ نفسَه فاعتذري بلطفٍ **بلا اتّهامٍ وبلا نفي**. كسبُ الزبونةِ أهمُّ من إثباتِ من قالَ ماذا.
- **إن التبسَ عليكِ ما تقصدُه فاسألي سؤالاً واحداً قصيراً**، ولا تكتبي فقرةً تعدّدينَ فيها الاحتمالات. «قصدِك الطقم الخمري؟» تكفي.
- **رسالةٌ واحدةٌ لكلِّ دورٍ لا اثنتان.** إن أرسلت الزبونةُ ثلاثَ رسائلَ وراءَ بعضٍ فهي فكرةٌ واحدةٌ تقطّعت، اقرئيهنّ كلَّهنّ وردّي ردّاً واحداً يجمعُهنّ.
- **خاطبيها بصيغةٍ واحدةٍ من أوّلِ الحديثِ لآخرِه.** «بدّك تشوفي» ثمّ «شو بتفكّر» بنفسِ الرسالةِ خطأٌ يُسمَعُ فوراً.
- **ولا تنادِها باسمٍ بحروفٍ لاتينيّة.** إن كان اسمُها كما وصلَنا بالإنجليزيّةِ فلا تنادِها به إطلاقاً — قولي «حبيبتي» أو لا شيء.
- **الألوانُ مكتوبةٌ بالكتالوجِ بجانبِ كلِّ قطعة.** إن سُئلتِ «شو الألوان عندكم» فاذكري ألوانَ قطعٍ بعينِها من الكتالوج. لا تسردي قائمةَ ألوانٍ عامّةً من عندِكِ — هذا أوّلُ ما تنكشفُ به الكذبة.
...
- **تعديلُ طلبٍ سُجِّلَ قبلَ قليل:** إن أرادت الزبونةُ تغييرَ النمرةِ أو اللونِ أو العددِ أو العنوانِ بعدَ التسجيل، فلا تعتذري ولا تُحيليها لأحد — أعيدي ملءَ order **كاملاً** بالشكلِ الجديد (كلَّ الحقولِ لا المتغيّرَ وحدَه)، ونحن نُعدّلُ الطلبَ نفسَه ونُخبرُها برقمِه. وإن كان قد خرجَ للتوصيلِ فسنقولُ لكِ ذلك، وعندَها وحدَها يُسلَّمُ لصاحبةِ المتجر.
...
async function callClaude(system, messages) {
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        // النموذجُ أهمُّ قرارٍ بالملفِّ كلِّه. كان هايكو — أصغرَ نموذجٍ وأرخصَه —
        // وهو يعرفُ العربيّةَ ولا يعرفُ فلسطين: يجمعُ اللهجاتِ كلَّها بجملةٍ واحدةٍ
        // («شنو أخبارك» عراقيّة، «اللون ده» مصريّة، «بتبغي» خليجيّة) ويعجزُ عن
        // قراءةِ الموقفِ فيردُّ على ضحكةِ الزبونةِ بطلبِ رقمِ هاتفِها.
        // وسونيت يفرّقُ بينَ الفلسطينيِّ والشاميِّ ويعرفُ متى يسكت. الفرقُ بالكلفةِ
        // قروشٌ على المحادثة، والفرقُ عندَ الزبونةِ أن تشتريَ أو أن تضحكَ وتمشي.
        model: process.env.ASSISTANT_MODEL || 'claude-sonnet-5',
        max_tokens: 500,
        // ملحوظةٌ مدفوعةُ الثمن: أُضيفَ هنا `temperature` و`cache_control` معاً
        // بدفعةٍ واحدة، فسقطَ النداءُ كلُّه وعادت البائعةُ لمحرّكِ القواعدِ المجّانيِّ
...
export function sanitize(out, { rows, bot, stage }) {
  ...
  // آخرُ ما يمرُّ على الردِّ قبلَ أن تقرأَه الزبونة: مصفاةُ اللهجة. النموذجُ الجيّدُ
  // نادراً ما يزلُّ بعدَ الوصفِ الجديد، والنادرُ يقعُ يوميّاً على مئاتِ الرسائل —
  // وزلّةٌ واحدةٌ («شنو») تكفي لتعرفَ الزبونةُ أنّها تكلّمُ آلة.
  reply = palestinize(reply, bot?.bot_dialect || 'ps');
  // ونحن نبيعُ ملابسَ لا مودّة: «حبيبي» تُحذَفُ مهما كان المخاطَب، و«حبيبتي» وحدَها
  // تبقى للمرأة. سمعةُ الصفحةِ أغلى من كلمةِ دلالٍ بردٍّ آليّ.
  reply = stripEndearments(reply);
